...
require(['$'], function() {
	$(function() {
	    ...
	    if(pvarIsLogin == "true"){
	    	var dataURL ="/kr/ko/life/event/evt20190201/checkTime";
	    	...
	    	$.ajax({
					...
					success : function(data) {
						var cnt = 0;
						$('.osulloc_calc_tbl .calTd').each(function(index,value) {
				            var me = $(this);
				            var day = parseInt(me.find('em').text());
				            day = (day < 10) ? '0' + day : day;
				            if (data.list.indexOf('2019' +'02'+ day+'') >= 0) {
				            	me.addClass("ok");
				            	cnt +=1;
				            }
				        });
			...
			var data = "today="+chan_val+"&seq="+evtId+"&method=evtApply";
				...
				$.ajax({
					...
					success : function(response) {
						...
						//appBoy 통계
						appboy.logCustomEvent("event_20190201_ok");
...
function callAjax(data,couponMsg){
	$.ajax({
		type : "POST",
		url : "/kr/ko/life/event/evt20190201/coupon",
		cache : false,
		data : "couponSeq="+data,
		dataType : "json",
		success : function(response) {
